...
namespace screen {
	...
	class circle : public primitive {
		...
		circle(circle const &another)
		{
			this->x0 = another.x0;
			this->y0 = another.y0;
			this->width = another.width;
			this->height = another.height;
			this->angle1 = another.angle1;
			this->angle2 = another.angle2;
		}
		...
		{
			int nscr = DefaultScreen(dsp);
			XDrawArc(dsp, wnd, DefaultGC(dsp, nscr), x0, y0, width,
					height, angle1, angle2);
		}

		private:
			int width, height, angle1,
			    angle2, x0, y0;
	};
...
void put_circle(int x0, int y0, int radius)
{
	screen::circle cl(x0, y0, radius);
	main_screen->add_primitive(cl);
}
